...
@Deprecated
public class OldKickCommand extends CommandHandler {
	private static Kick kick;

	public OldKickCommand(final Kick kickModule) {
		super(kickModule);
		kick = kickModule;
	}
	...
	public static class KickCmd extends BATCommand {
		...
		public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
            ...
            Preconditions.checkArgument(args.length != 1 || !BungeeAdminToolsPlugin.getInstance().getConfiguration().isMustGiveReason(),
	    	...
	    	if(player != null){
	    		final String pServer = player.getServer().getInfo().getName();
   				...
   				BungeeAdminToolsPlugin.broadcast(returnedMsg, PermissionManager.Action.KICK_BROADCAST.getPermission());
	    	}else{
		    	...
		    	returnedMsg = kick.kickSQL(pUUID, RedisBungee.getApi().getServerFor(pUUID).getName(), sender.getName(), 
		    		(args.length == 1) ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
	    	    BungeeAdminToolsPlugin.getInstance().getRedis().sendMoveDefaultServerPlayer(pUUID);
    	    	BungeeAdminToolsPlugin.broadcast(returnedMsg, PermissionManager.Action.KICK_BROADCAST.getPermission());
	    	}
		}
	}
			...
			if (BungeeAdminToolsPlugin.getInstance().getRedis().isRedisEnabled()) {
			    	...
			    	if (player != null) {
				    	...
				    	returnedMsg = kick.gKickSQL(pUUID, sender.getName(),
				    		(args.length == 1) ? IModule.NO_REASON : Utils.getFinalArg(args, 1));
			    	        BungeeAdminToolsPlugin.getInstance().getRedis().sendGKickPlayer(pUUID, returnedMsg);
			    	}
		    	    	BungeeAdminToolsPlugin.broadcast(returnedMsg, PermissionManager.Action.KICK_BROADCAST.getPermission());
			} else {
			final ProxiedPlayer player = ProxyServer.getInstance().getPlayer(pName);
