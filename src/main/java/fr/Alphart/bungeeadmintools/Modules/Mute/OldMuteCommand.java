...
@Deprecated
public class OldMuteCommand extends CommandHandler {
	private static Mute mute;

	public OldMuteCommand(final Mute muteModule) {
		super(muteModule);
		mute = muteModule;
	}
	...
	public static void handleMuteCommand(final BATCommand command, final boolean global, final boolean ipMute,
        ...
        checkArgument(
                    !reason.equalsIgnoreCase(IModule.NO_REASON) || !BungeeAdminToolsPlugin.getInstance().getConfiguration().isMustGiveReason(),
		...
		if (ipMute && !BungeeAdminToolsPlugin.getInstance().getRedis().isRedisEnabled() && player != null) {
			returnedMsg = mute.muteIP(player, server, staff, 0, reason);
		} else {
			returnedMsg = mute.mute(target, server, staff, 0, reason);
		}

		BungeeAdminToolsPlugin.broadcast(returnedMsg, PermissionManager.Action.MUTE_BROADCAST.getPermission());
	}
