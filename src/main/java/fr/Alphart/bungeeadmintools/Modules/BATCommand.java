...
public abstract class BATCommand extends net.md_5.bungee.api.plugin.Command implements TabExecutor {
        ...
        if (exception instanceof IllegalArgumentException) {
            if (exception.getMessage() == null) {
                if (coreCommand) {
                    ...
                    sender.sendMessage(I18n.formatWithColorAndAddPrefix("invalidArgsUsage", new String[]{"&e/bat " + getFormatUsage()}));
                } else {
                    sender.sendMessage(I18n.formatWithColorAndAddPrefix("invalidArgsUsage", new String[]{"&e/" + getFormatUsage()}));
                }
            } else if (formatWithColor("noPerm").equals(exception.getMessage())) {
                sender.sendMessage(formatWithColorAndAddPrefix("noPerm"));
            } else {
                sender.sendMessage(I18n.formatWithColorAndAddPrefix("invalidArgs", new String[]{exception.getMessage()}));
            }
    ...
    public void mustConfirmCommand(final CommandSender sender, final String command, final String message) {
        ...
        if (!CommandQueue.isExecutingQueueCommand(sender)) {
            if ("".equals(message)) {
                sender.sendMessage(I18n.formatWithColorAndAddPrefix("mustConfirm", new String[]{"", cmdToConfirm}));
            } else {
                sender.sendMessage(I18n.formatWithColorAndAddPrefix("mustConfirm", new String[]{"&e" + message, cmdToConfirm}));
            }
